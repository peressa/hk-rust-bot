...
import { LayoutDashboard, MessageSquare, Map as MapIcon, Video, Settings, LogOut, Radio } from "lucide-react";
    ...
    <div className="dashboard-container" style={{ display: 'flex', minHeight: '100vh', background: 'var(--background)' }}>
      ...
      <aside className="glass-panel" style={{ 
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1 }}>
          ...
          <SidebarItem icon={<MapIcon size={20} />} label="Mapa táctico" href="/dashboard/map" />
          <SidebarItem icon={<MessageSquare size={20} />} label="Chat de equipo" href="/dashboard/chat" />
          <SidebarItem icon={<Video size={20} />} label="Cámaras CCTV" href="/dashboard/cameras" />
        </nav>

        <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <SidebarItem icon={<Settings size={20} />} label="Configuración" href="/dashboard/settings" />
          <SidebarItem icon={<LogOut size={20} />} label="Cerrar sesión" href="/api/auth/signout" />
        </div>
      </aside>
